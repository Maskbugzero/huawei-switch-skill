...
# -*- coding: utf-8 -*-
"""
S5735R PoE 配置部署脚本 - 针对 COM4 口
自动从 .txt 文件读取最新配置（推荐方式）
自动备份 + 部署 + save
"""
...
logger = get_logger("deploy_poe_com4")
...
def deploy_poe_to_com4(password: str, config_file: str, backup_first: bool = True):
    """部署 PoE 配置到 COM4（自动读取最新 .txt 文件）"""
    device_name = "S5735R-COM4-PoE"
    ...
    logger.info("=== 开始 S5735R PoE 配置部署 (COM4) ===")
    ...
    try:
        ...
        conn = Connection(port=port, password=password, timeout=60)
        conn.connect()
        ...
        logger.info("=== 部署成功完成 ===")
        logger.info("请检查交换机状态（IP、sysname 等已按最新配置更新）")
        return True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="S5735R PoE 配置部署工具 (COM4) - 自动读取 .txt 文件"
    )
    ...
    parser.add_argument("--device-name", default="S5735R-COM4-PoE", help="设备名称")

    parser.add_argument(
        "--config-file",
        "-c",
        default=r"D:\work\0731\S5735R poe.txt",
        help="配置文件路径（默认: D:\\work\\0731\\S5735R poe.txt）"
    )
